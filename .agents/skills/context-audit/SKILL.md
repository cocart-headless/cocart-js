...
name: context-audit
description: >
  Audit your Codex setup for token waste and context bloat. Use when
  the user says "audit my context", "check my settings", "why is Codex so
  ...
  AGENTS.md rules, skills, settings, and file permissions. Returns a
...
Read all AGENTS.md files (project root, .Codex/, ~/.Codex/).
Count lines. Then read every rule and test against five filters:

| Filter | Flag when... |
|--------|-------------|
| Default | Codex already does this without being told ("write clean code", "handle errors") |
...
If total AGENTS.md lines > 200, check for progressive disclosure
...
actually bloated -- a lean AGENTS.md with universal context is fine
as a single file.

### Skills

Scan .Codex/skills/*/SKILL.md. For each skill:
...
| Issue | Points |
|-------|--------|
| AGENTS.md > 200 lines | -10 |
| AGENTS.md > 500 lines | -20 |
...
- Show you a cleaned-up AGENTS.md with the flagged rules removed
...
Show diffs for AGENTS.md and skills -- let the user confirm before
